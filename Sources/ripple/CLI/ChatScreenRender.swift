...
extension ChatScreen {
    ...
    private func workingLine() -> String {
        ...
        let label = compacting ? "compacting context…"
            : (loading ? "switching model…" : String(format: "\(verb) %.1fs", elapsed))
        ...
        // chunk count isn't a real token rate - only show tok/s for on-device variants.
        if !variant.isRemote, let assistant = liveAssistant, elapsed > 0.3, assistant.tokenCount > 0 {
            line += Paint.fg(238, "  ·  ") + Paint.fg(244, "\(Int(Double(assistant.tokenCount) / elapsed)) tok/s")
        }
    ...
    /// `/model` switch fetches weights). esc cancels it.
    private func downloadLine(_ download: DownloadProgress) -> String {
