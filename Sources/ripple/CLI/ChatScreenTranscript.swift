...
extension ChatScreen {
    ...
    private func assistantLines(_ a: Assistant, width: Int) -> [Line] {
        ...
        // A failed run renders its error under whatever did stream - never a silent empty turn.
        if let failure = a.failure {
            if !out.isEmpty { out.append(Line("")) }
            for (index, line) in wrap("turn failed - " + failure, width - 6).enumerated() {
                out.append(Line("  " + Paint.fg(174, (index == 0 ? "✗ " : "  ") + line)))
            }
        }
        out.append(Line(""))
        return out
