...
extension ChatScreen {
    ...
    func switchToVariant(_ choice: DeepAgentVariant) {
        ...
        startSpinner()
        requestRender()
        Task { await self.performSwitch(choice) }
    }

    /// The switch itself, split out of ``switchToVariant(_:)`` so tests can await it. A failed
    /// rebuild keeps the current agent running and says so in the transcript - a switch that
    /// silently stays put reads as a hang.
    func performSwitch(_ choice: DeepAgentVariant) async {
        if let newAgent = await build(choice, policy) {
            ...
            messages.append(Message(kind: .note(
                "Could not switch to \(choice.label) - its planner model isn't available. Still on \(variant.label)."
            )))
            invalidateTranscriptCache()
        }
        loading = false
