...
public enum DeepAgentREPL {
    public static func run(
        ...
        // Warm the planner behind a progress bar before entering the full-screen UI; `build` then
        // reuses the cached container instantly. A tty is guaranteed (checked above), so the bar's
        // carriage-return redraws render cleanly. A load failure falls back rather than bailing -
        // see `startupAgent`.
        banner(variant)
        guard let startup = await startupAgent(
            variant: variant, manager: manager, openAIModels: openAIModels, build: build, policy: policy
        ) else { return }
        let active = startup.variant
        let screen = ChatScreen(
            variant: active, agent: startup.agent, build: build, gate: gate, askGate: askGate, variants: variants,
            ...
            // Label an in-turn lazy (re)load - e.g. after an idle-unload - as the model loading, so
            // the wait isn't misread as slow prompt processing.
            modelLoadStatus: { manager.loadingModelID.map(ChatScreen.name) }
        )
        ...
        // A launch-time model problem (load failure, fallback taken) opens the session with a note
        // in the transcript - the pre-TUI stderr line is wiped by the full-screen switch.
        if let note = startup.note { screen.messages.append(Message(kind: .note(note))) }
        await screen.run()
    ...
    /// Warm the chosen planner and build the agent for launch, degrading instead of dying: a model
    /// problem must not keep the whole session from starting when another planner can. On a load
    /// failure the default planner takes over when it's a different model already on disk; when it
    /// isn't, the choice is kept anyway - its lazy first-turn load retries and reports into the
    /// transcript (see `Assistant.failure`), and `/model` can switch. An unresolvable planner (an
    /// unknown `--model` id) likewise falls back to the default. Every degradation is described in
    /// `note`, shown in the transcript - the pre-TUI stderr lines are wiped by the full-screen
    /// switch. Returns nil only when no usable planner exists at all (the error is printed).
    ///
    /// On-device variants warm the planner behind a progress bar; the vision model is deliberately
    /// NOT warmed - it loads lazily on first use (and idle-unloads), so an agent that never looks
    /// at the screen never pays for it. A remote variant has nothing to warm.
    private static func startupAgent(
        variant: DeepAgentVariant, manager: MlxModelLoader, openAIModels: [OpenAIModelConfig],
        build: ChatScreen.Build, policy: AgentToolPolicy
    ) async -> (variant: DeepAgentVariant, agent: ReactAgent, note: String?)? {
        var active = variant
        var note: String?
        let fallback = RippleModelResolution.resolveVariant(nil, remote: openAIModels)
        if !variant.isRemote, await loadWithProgress(manager, variant.textModelID, role: "main agent") == nil {
            let reason = manager.lastLoadError.map { " (\($0))" } ?? ""
            let chosen = ChatScreen.name(variant.textModelID)
            if fallback.textModelID != variant.textModelID,
               ModelCache.missing([fallback.textModelID]).isEmpty,
               await loadWithProgress(manager, fallback.textModelID, role: "main agent") != nil {
                active = fallback
                note = "\(chosen) failed to load\(reason) - started on "
                    + ChatScreen.name(fallback.textModelID) + " instead. Switch models with /model."
            } else {
                note = "\(chosen) failed to load\(reason) - sending a message retries it, "
                    + "or pick another model with /model."
            }
        }
        if let agent = await build(active, policy) { return (active, agent, note) }
        // The planner can't even be resolved (an unknown `--model` id, a remote entry whose
        // registration vanished) - fall back to the default variant when it's a different,
        // on-disk model.
        if fallback.textModelID != active.textModelID,
           ModelCache.missing([fallback.textModelID]).isEmpty,
           let agent = await build(fallback, policy) {
            let switched = "\(active.label) isn't available - started on "
                + ChatScreen.name(fallback.textModelID) + " instead. Switch models with /model."
            return (fallback, agent, switched)
        }
        let reason = manager.lastLoadError.map { ": \($0)" } ?? " - no usable planner model."
        err("failed to load the models\(reason)")
        return nil
    }
    ...
    static func loadWithProgress(
        ...
        // Say *why* it failed - a blanket "failed" hides everything from a half-fetched snapshot to
        // an unsupported architecture.
        if !ok, let reason = loader.lastLoadError {
            err(reason)
        }
        return ok ? model : nil
    }
