...
struct OpenAIModelConfig: Sendable, Hashable {
    ...
    private func bedrockModel() -> BedrockChatModel? {
        guard let credentials = BedrockCredentials.fromEnvironment() else { return nil }
        return BedrockChatModel(
            region: resolvedRegion, model: model, credentials: credentials, supportsVision: vision,
            modelID: name, contextWindowTokens: resolvedContextWindow,
            parameters: AnthropicGenerateParameters(maxTokens: maxTokens, temperature: temperature, topP: topP)
        )
    }
...
/// `https://api.openai.com/v1`. Bedrock reads AWS credentials from the standard `AWS_ACCESS_KEY_ID`
