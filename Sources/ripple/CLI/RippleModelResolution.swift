...
enum RippleModelResolution {
    ...
    static func lazyChatModel(
        ...
        return LazyChatModel(
            supportsVision: model.loadsAsVision, modelID: model.id,
            contextWindowTokens: model.contextWindowTokens,
            begin: {
                guard let chat = await manager.beginUse(id) else {
                    // Carry the loader's reason (a half-fetched snapshot, an unsupported
                    // architecture) so the turn's failure line says why, not just which model.
                    throw await RippleModelError.unavailable(id, reason: manager.lastLoadError)
                }
                return chat
            },
...
/// A configured deep-agent model id that isn't in the catalog or fails to load. `LocalizedError`
/// so the agent's `.failed` event (and the TUI's error line) carries the readable reason instead
/// of a generic NSError rendering.
enum RippleModelError: LocalizedError {
    case unavailable(String, reason: String? = nil)

    var errorDescription: String? {
        switch self {
        case .unavailable(let id, let reason):
            "\(id) could not be loaded" + (reason.map { ": \($0)" } ?? "")
        }
    }
}
