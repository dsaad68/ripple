...
!!! note "Larger models"
    The 1.2B model runs comfortably on 16 GB unified memory, as does the 2.6B in its `mxfp4` or `mxfp8` precision (`LiquidAI/LFM2.5-2.6B-MLX/mxfp4`). If you have 32 GB or more, the MoE variant (`LiquidAI/LFM2.5-8B-A1B-MLX-8bit`) handles longer context and more complex reasoning tasks. See [Local MLX models](../models/local.md) for the full catalog and memory guidance.
