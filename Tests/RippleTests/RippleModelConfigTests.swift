...
struct RippleModelConfigTests {
    ...
    @Test func bedrockChatModelRequiresAWSCredentials() {
        ...
        // Clear AWS_BEARER_TOKEN_BEDROCK too, so a stray bearer token in the ambient env can't flip
        // the SigV4 fallback path under test.
        withEnvironment(["AWS_ACCESS_KEY_ID": nil, "AWS_SECRET_ACCESS_KEY": nil, "AWS_BEARER_TOKEN_BEDROCK": nil]) {
            #expect(config.chatModel() == nil) // no AWS creds and no bearer token in the environment
        }
        withEnvironment(["AWS_ACCESS_KEY_ID": "AK", "AWS_SECRET_ACCESS_KEY": "SK", "AWS_BEARER_TOKEN_BEDROCK": nil]) {
            #expect((config.chatModel() as? BedrockChatModel) != nil) // SigV4 from access-key credentials
        }
    }

    @Test func bedrockBuildsFromBearerTokenInAPIKey() {
        let config = OpenAIModelConfig(
            name: "b", baseURL: "https://bedrock-runtime.us-east-1.amazonaws.com",
            model: "us.anthropic.claude-opus-4-8", apiKey: "bearer-tok", vision: false,
            reasoning: false, temperature: nil, maxTokens: nil, topP: nil,
            provider: .bedrock, region: "us-east-1"
        )
        // A bearer token in `apiKey` builds the model even with no AWS access-key credentials.
        withEnvironment(["AWS_ACCESS_KEY_ID": nil, "AWS_SECRET_ACCESS_KEY": nil, "AWS_BEARER_TOKEN_BEDROCK": nil]) {
            #expect((config.chatModel() as? BedrockChatModel) != nil)
        }
    }

    @Test func bedrockBuildsFromBearerTokenEnvVar() {
        let config = OpenAIModelConfig(
            name: "b", baseURL: "https://bedrock-runtime.us-east-1.amazonaws.com",
            model: "us.anthropic.claude-opus-4-8", apiKey: nil, vision: false,
            reasoning: false, temperature: nil, maxTokens: nil, topP: nil,
            provider: .bedrock, region: "us-east-1"
        )
        withEnvironment(["AWS_ACCESS_KEY_ID": nil, "AWS_SECRET_ACCESS_KEY": nil, "AWS_BEARER_TOKEN_BEDROCK": "env-tok"]) {
            #expect((config.chatModel() as? BedrockChatModel) != nil)
        }
    }

    @Test func bedrockBearerTokenRequiresBaseURL() {
        let config = OpenAIModelConfig(
            name: "b", baseURL: "", model: "us.anthropic.claude-opus-4-8", apiKey: "bearer-tok",
            vision: false, reasoning: false, temperature: nil, maxTokens: nil, topP: nil,
            provider: .bedrock, region: "us-east-1"
        )
        // Bearer auth needs an explicit, verbatim baseURL; without one the model can't be built.
        withEnvironment(["AWS_ACCESS_KEY_ID": nil, "AWS_SECRET_ACCESS_KEY": nil, "AWS_BEARER_TOKEN_BEDROCK": nil]) {
            #expect(config.chatModel() == nil)
        }
    }

    @Test func bedrockAuthResolutionPrecedence() {
        // Explicit token (the config apiKey) wins over the env bearer token and SigV4 credentials.
        withEnvironment([
            "AWS_BEARER_TOKEN_BEDROCK": "env-token", "AWS_ACCESS_KEY_ID": "AK",
            "AWS_SECRET_ACCESS_KEY": "SK", "AWS_SESSION_TOKEN": nil
        ]) {
            #expect(BedrockAuth.resolve(bearerToken: "explicit") == .bearerToken("explicit"))
            // With no explicit token, the env bearer token beats SigV4 credentials.
            #expect(BedrockAuth.resolve() == .bearerToken("env-token"))
        }
        // No bearer token anywhere -> SigV4 from the access-key pair.
        withEnvironment([
            "AWS_BEARER_TOKEN_BEDROCK": nil, "AWS_ACCESS_KEY_ID": "AK",
            "AWS_SECRET_ACCESS_KEY": "SK", "AWS_SESSION_TOKEN": nil
        ]) {
            #expect(BedrockAuth.resolve() == .sigV4(BedrockCredentials(accessKey: "AK", secretKey: "SK")))
        }
        // Nothing set -> nil.
        withEnvironment([
            "AWS_BEARER_TOKEN_BEDROCK": nil, "AWS_ACCESS_KEY_ID": nil,
            "AWS_SECRET_ACCESS_KEY": nil, "AWS_SESSION_TOKEN": nil
        ]) {
            #expect(BedrockAuth.resolve() == nil)
        }
    }
}
