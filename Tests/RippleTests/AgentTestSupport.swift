...
/// Serializes tests that mutate the process-global environment. Swift Testing runs tests in
/// parallel, but `setenv`/`getenv` touch process-wide state that isn't thread-safe, so two env
/// tests at once race on each other's values (e.g. a shared `${VAR}` resolving to the wrong
/// secret) and can even crash. `withEnvironment` holds a process-wide lock, applies `vars` (a nil
/// value removes the key), runs `body`, then restores the previous values - so env tests are
/// mutually exclusive and leave no residue behind.
private let environmentTestLock = NSLock()

func withEnvironment<R>(_ vars: [String: String?], _ body: () throws -> R) rethrows -> R {
    environmentTestLock.lock()
    defer { environmentTestLock.unlock() }
    let previous = vars.keys.map { key in (key, getenv(key).map { String(cString: $0) }) }
